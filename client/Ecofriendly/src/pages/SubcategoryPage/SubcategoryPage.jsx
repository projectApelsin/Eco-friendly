...
const SubcategoryPage = () => {
  const { subcategoryId } = useParams(); // Получаем параметр запроса из URL
  ...
  useEffect(() => {
    const fetchSearchResults = async () => {
      ...
      setError(null);
      try {
        const response = await ApiConfig.get(`/api/public/subcategoryPage/${subcategoryId}`);
